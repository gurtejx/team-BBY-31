...
app.post("/submitUser", async (req, res) => {
  ...
  const usernameResult = await userCollection
    .find({
      username: username,
    })
    .project({
  ...
  const emailResult = await userCollection
    .find({
      email: email,
    })
    .project({
...
app.post("/respond", async (req, res) => {
  ...
  try {
    ...
    const response = await axios.post(
      "http://lodxzqsita.eu10.qoddiapp.com/respond",
      {
        prompt: prompt,
        language: language,
      }
...
app.get("/fetchProfile", sessionValidation, async (req, res) => {
  try {
    const user = await userCollection.findOne(
      ...
      { projection: { name: 1, username: 1, email: 1, profession: 1 } }
    );
    res.json({ user });
  } catch (err) {
    console.error(err);
...
app.post("/updatePassword", async (req, res) => {
  ...
  if (
    newPasswordValidation.error != null ||
    confirmPasswordValidation.error != null
  ) {
...
app.post("/pdf", async (req, res) => {
  ...
  try {
    // Make an HTTP POST request to the Python backend
    const response = await axios.post(
      "http://lodxzqsita.eu10.qoddiapp.com/setupPDF",
      {
        data: pdfData,
      }
    );
...
app.post("/askQuestion", async (req, res) => {
  ...
  try {
    ...
    const response = await axios.post(
      "http://lodxzqsita.eu10.qoddiapp.com/askQuestion",
      {
